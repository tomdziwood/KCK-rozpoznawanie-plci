...
def main():
    ...
    #for iTytul in [2]:
        ...
        while((i <= 20) and (len(indeksy) > 0) and (indeksy[0] + fragmentSzerokosc < dlugoscSygnalu)):
            ...
            #print("np.argmax(sygnalHz) = " + str(np.argmax(sygnalHz)))
            #dominujacaCzestotliwosc = freqs[np.argmax(sygnalHz)]
            ...
            if(maxMalySygnalHz / maxFragmentSygnal < 0.1):
                ...
                j = 1
                while((j < len(malySygnalHz)) and (malySygnalHz[j] - malySygnalHz[j - 1] < 0.2 * maxMalySygnalHz)):
                    j += 1
